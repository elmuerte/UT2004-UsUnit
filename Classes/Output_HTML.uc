/*******************************************************************************
	...
	<!-- $Id: Output_HTML.uc,v 1.3 2005/06/21 20:19:30 elmuerte Exp $ -->
*******************************************************************************/
...
function testEnd(TestBase test)
{
	...
	{
		html.Logf(indent$"    <p class=\"testStats\">");
		if (TestSuite(Test).isAborted()) html.Logf("<em>Test suite was aborted because a test failed</em>;");
		html.Logf(indent$"Success: "$string(test.getSuccessPct())$"%</p>");
	}
	html.Logf(indent$"    </td>");
	html.Logf(indent$"</tr>");
	html.Logf(indent$"</table>");
...
function _head()
{
	...
	html.Logf("    <meta name=\"generator\"/ content=\"$Id: Output_HTML.uc,v 1.3 2005/06/21 20:19:30 elmuerte Exp $\">");
	_style();
	html.Logf("</head>");
	html.Logf("<body>");
...
function _style()
{
	...
	html.Logf("    TD.suite { background-color: #999999; }");
	...
	html.Logf("    SPAN.file { font-family: monospace; padding-left: 25px; }");
