...
/*******************************************************************************
    ...
    <!-- $Id: UsUnitUtils.uc,v 1.12 2005/09/11 11:48:56 elmuerte Exp $ -->
*******************************************************************************/

class UsUnitUtils extends Object config(UsUnit);

/** cached list with known subclasses */
var config array< class<TestBase> > KnownTestClasses;
...
event Created()
{
    ...
    {
        if (KnownTestClasses[i] == none) // class couldn't be resolved, remove
        {
            KnownTestClasses.remove(i, 1);
            continue;
        }
...
final static operator(24) bool > (class<TestBase> A, class<TestBase> B)
{
    return caps(string(A)) > caps(string(B));
}
...
function AddKnownTestClass(class<TestBase> cls, optional bool bDontSave)
{
    local int i;
    //TODO: improve search
